{{- define "marshal.func" }}
func marshal{{ .Type | splitList "." | last | snakecase | camelcase -}}
...
{{ range $n, $f := .Fields }}
		...
		{{- if eq $f.Kind "struct" }}
			v := marshal{{ $f.Type | splitList "." | last | snakecase | camelcase -}}
				(top.([]interface{}))
		{{- else }}
			v := top.({{ $f.Type }})
		...
		{{- else if $f.IsSlice }}
		{{- if eq $f.Kind "struct" }}
		item.{{ $n }} = marshal{{ $f.Type | splitList "." | last | snakecase | camelcase -}}
...
{{ range $n, $f := .Fields }}
	...
	{{- else if $f.IsSlice }}
	{{- if eq $f.Kind "struct" }}
	data.{{ $n }} = marshal{{ $f.Type | splitList "." | last | snakecase | camelcase -}}
		(c[{{ $f.Name | quote }}].([]interface{}))
	{{- else if not (eq $f.Kind $f.Type) }}
...
func Marshal{{ .Schema.Type | splitList "." | last | snakecase | camelcase -}} (d map[string]interface{}) {{ .Schema.Type }} {
	data := {{ .Schema.Type }}{}
